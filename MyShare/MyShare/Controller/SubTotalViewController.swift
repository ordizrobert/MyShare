...
class SubTotalViewController: BaseViewController {
    ...
    lazy var topLabel:UILabel = {
        ...
        topLabel.text = LabelType.subTotalAmout.rawValue
    ...
    override func setupViews() {
        ...
        logoImage.image = UIImage(named: "darkSubTotalicon")
    ...
    @objc func doneAction() {
        self.done()
        let total: String = "\(inputValuesTextField.text?.dropFirst() ?? "")"
        if inputValuesTextField.text!.count > 0 && total.isInt == true {
            let viewController = TaxViewController()
            viewController.totalGroup = self.totalGroup
            viewController.subTotal = Double(total)!
